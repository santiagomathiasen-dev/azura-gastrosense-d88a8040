...
export function useProfile() {
    ...
    const { data: profile, isLoading, error, refetch } = useQuery({
        ...
        // Never throw to the global error boundary — a missing profile must not crash the app.
        throwOnError: false,
        queryFn: async () => {
            ...
            try {
                ...
                try {
                    ...
                    // Ignore abort/signal errors — transient in React StrictMode double-mount.
                    const isAbortLike =
                        fetchErr?.name === 'AbortError' ||
                        fetchErr?.message?.includes('aborted') ||
                        fetchErr?.message?.includes('signal') ||
                        fetchErr?.message?.includes('expirou');
                    if (isAbortLike) {
                        console.warn("useProfile: request aborted (StrictMode cleanup), returning null.");
                        return null;
                    }
