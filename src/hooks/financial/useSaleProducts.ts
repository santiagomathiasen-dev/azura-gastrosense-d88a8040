...
// FASE 4 + EXTRAÇÃO: importa da API pública do módulo OPS
    ...
    mutationFn: async ({ sale_product_id, quantity = 1 }: { sale_product_id: string; quantity?: number }) => {
      ...
      for (const component of product.components || []) {
        ...
        if (component.component_type === 'finished_production') {
          // FASE 4: Usa OpsReadService para verificar + OpsStockService para deduzir
          const stock = await OpsReadService.getFinishedStockBySheet(component.component_id);
          const availableQty = stock ? Number(stock.quantity) : 0;

          if (!stock || availableQty < neededTotalQty) {
            const sheet = await OpsReadService.getSheetSummary(component.component_id);
            insufficientItems.push({
              id: component.component_id,
              name: sheet?.name || 'Producao desconhecida',
              type: 'finished_production',
              amount: Math.max(0, neededTotalQty - availableQty),
            });
            continue;
          }

          await OpsStockService.deductFinishedStock(component.component_id, neededTotalQty);

        } else if (component.component_type === 'stock_item') {
          // FASE 4: Usa OpsReadService para verificar + OpsStockService para deduzir
          const prodStock = await OpsReadService.getProductionStockByItem(component.component_id);
          const availableProdQty = prodStock ? Number(prodStock.quantity) : 0;

          if (!prodStock || availableProdQty < neededTotalQty) {
            insufficientItems.push({
              id: component.component_id,
              name: prodStock?.stock_item?.name || 'Item desconhecido',
              type: 'stock_item',
              amount: Math.max(0, neededTotalQty - availableProdQty),
            });
            continue;
          }

          await OpsStockService.updateProductionStock(component.component_id, -neededTotalQty, ownerId);
