...
export function usePurchaseCalculationByPeriod({ productions }: UsePurchaseCalculationByPeriodParams) {
  ...
  // Calculate projected consumption for productions in the given period
  const getProjectedConsumption = useMemo(() => {
  ...
  return {
    ...
    urgentCount,
    totalEstimatedCost,
    isLoading: stockLoading || suppliersLoading || productionStockLoading,
