  ...
  const subtractStockForProduction = async (production: ProductionWithSheet) => {
    ...
    for (const ingredient of production.technical_sheet.ingredients) {
      ...
      if (plan.fromCentral > 0) {
        // FASE 5 (dual-write): ainda grava related_production_id para compat retroativa.
        // Apos rodar FASE5_drop_fk.sql remover a linha related_production_id abaixo.
        const movResult = await supabaseFetch('stock_movements', {
          method: 'POST',
          ...
          body: JSON.stringify({
            ...
            notes: `Baixa automatica - Producao: ${production.name}`,
          })
        });

        // FASE 5: registra o vinculo na nova tabela de juncao (sem FK circular)
        const movement = Array.isArray(movResult) ? movResult[0] : movResult;
        if (movement?.id) {
          try {
            await supabaseFetch('production_stock_events', {
              method: 'POST',
              body: JSON.stringify({
            ...
            // Nao bloqueia o fluxo principal — tabela pode ainda nao existir no banco
            console.warn('[FASE5] production_stock_events insert falhou:', e);
          }
        }
      ...
      if (plan.insufficient > 0) {
        ...
        // FASE 2: Delegado ao ProductionService — hook de OPS não escreve mais em purchase_list_items
        await ProductionService.requestPurchase({
          ownerId,
          stockItemId,
          quantity: plan.insufficient,
