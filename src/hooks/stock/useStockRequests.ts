...
import { OpsStockService } from '@azura/ops';
...
export function useStockRequests() {
  ...
  const fulfillRequest = useMutation({
    mutationFn: async ({ requestId, deliverQuantity }: { requestId: string; deliverQuantity: number }) => {
      ...
      // FASE 4: Usa OpsStockService em vez de supabaseFetch direto
      await OpsStockService.updateProductionStock(request.stock_item_id, deliverQuantity, ownerId);
